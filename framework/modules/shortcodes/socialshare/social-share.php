...
class SocialShare implements ShortcodeInterface {
	...
	public function render($atts, $content = null) {
		...
		if ($params['enable_social_share']) {
			if ($params['enabled']) {
				$html .= goodwish_edge_get_shortcode_module_template_part('templates/' . $params['type'], 'socialshare', '', $params);
			}
		}
	...
	private function getSocialNetworksParams($params) {
		...
		foreach ($this->socialNetworks as $net) {
			...
			if (goodwish_edge_options()->getOptionValue('enable_'.$net.'_share') == 'yes') {
				...
				$html = goodwish_edge_get_shortcode_module_template_part('templates/parts/network', 'socialshare', '', $params);
		...
		switch ($net) {
            case 'facebook':
                if(wp_is_mobile()) {
                    ...
                    $link = 'window.open(\'http://www.facebook.com/sharer.php?s=100&amp;p[title]=' . urlencode(goodwish_edge_addslashes(get_the_title())) . '&amp;p[url]=' . urlencode(get_permalink()) . '&amp;p[images][0]=' . $image[0] . '&amp;p[summary]=' . urlencode(goodwish_edge_addslashes(get_the_excerpt())) . '\', \'sharer\', \'toolbar=0,status=0,width=620,height=280\');';
                }
                break;
            case 'twitter':
                ...
                if(wp_is_mobile()) {
                    ...
                    $link = 'window.open(\'http://twitter.com/home?status=' . urlencode(goodwish_edge_the_excerpt_max_charlength($count_char) . $twitter_via) . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');';
                }
                break;
			case 'google_plus':
			...
			case 'tumblr':
				...
				break;
			case 'pinterest':
				$link = 'popUp=window.open(\'http://pinterest.com/pin/create/button/?url=' . urlencode(get_permalink()) . '&amp;description=' . goodwish_edge_addslashes(get_the_title()) . '&amp;media=' . urlencode($image[0]) . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false;';
				break;
			case 'vk':
